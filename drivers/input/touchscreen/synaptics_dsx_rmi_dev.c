...
extern int remote_rmi4_i2c_read(unsigned short addr, unsigned char *data, unsigned short length);
extern int remote_rmi4_i2c_write(unsigned short addr, unsigned char *data, unsigned short length);
extern int remote_rmi4_i2c_enable(bool enable);
extern int remote_rmi4_get_irq_gpio(void);
extern int remote_rmit_set_page(unsigned int address);
extern int remote_rmit_put_page(unsigned int address);
extern struct input_dev *remote_rmi4_get_input(void);
extern struct i2c_client *remote_rmi4_get_i2c_client(void);
extern void remote_rmi4_delay_work(struct work_struct *work);
...
#endif
/* OPPO 2013-09-17 ranfei Modify end */
	},
	.size = 0,
...
void remote_rmi4_delay_work(struct work_struct *work) {
	rmidev_create_attr(true) ;
}

static int rmidev_init_device(void)
{
	int retval;
	dev_t dev_no;
	struct rmidev_data *dev_data;
...
static void rmidev_remove_device(void)
{
	struct rmidev_data *dev_data;
